...
export const TariffItem = styled.div<{ $isMobile: boolean }>`
  ...
  width: ${({ $isMobile }) => ($isMobile ? 'calc(100% - 5px)' : 'calc(33% - 10px)')};
`;

export const TariffItemPro = styled.div<{ $isMobile: boolean }>`
  margin-top: 1px;
  transition: 0.2s ease;
  display: flex;
  flex-direction: column;
...
export const TariffOldPrice = styled.p`
  transition: 0.2s ease;
